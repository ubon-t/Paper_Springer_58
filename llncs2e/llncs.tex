...
%
For example, the LPR system can be used in smart border safety to monitor vehicles entering and leaving the country, recording license plates with a location and date and time of entry using license plate readers.
%% 
These records are used to match the vehicle blacklist to detect the criminal/suspect vehicles crossing the border.
%%
In addition to that, LPR can be used in smart parking areas or smart toll stations to help facilitate the vehicles bearing authorized license plates that the gates is opened when the vehicles have authorization.
%%
The intellectualized traffic management and traffic safety enforcement also use LPR to implementing the processes.
%%
Furthermore, the security control of restricted areas can
improve the intruder prevention capability by using LPR to detect license plates authorization.
%%

For the vehicle recognition method, there are several methods such as vehicle type recognition,  vehicle color recognition,  vehicle brand recognition, and vehicle model recognition to help facilitate the vehicle detection process \cite{ShengITS}, \cite{AshtariITS}, \cite{ZezhiChen}, \cite{Friedman}, \cite{Stauffer}, \cite{Hsieh}.
%5
...
%
Section III explains the research testbed and research design. 
%
Experimental results are shown in Section IV, and discussed in Section V. 
%
Section VI concludes and presents future directions.
%
...
\bibitem{Ronak} Ronak P Patel, Narendra M Patel, Keyur Brahmbhatt: Automatic Licenses Plate Recognition. In: International Journal of Computer Science and Mobile Computing Vol.2 Issue. 4, April- 2013, pg. 285-294
